...
class Music(commands.Cog):
    ...
    async def play(
            ...
            manual_selection: bool = commands.Param(name="выбрать вручную",
                                                    description="Выберите песню вручную из найденных результатов",
                                                    default=False),
            options: PlayOpts = commands.Param(name="opções", description="Варианты обработки плейлиста",
                                               default=False),
            source: SearchSource = commands.Param(name="источник",
                                                  description="Выберите сайт для поиска музыки (не ссылок)",
                                                  default="ytsearch"),
            repeat_amount: int = commands.Param(name="повторения", description="установить количество повторений.",
                                                default=0),
            hide_playlist: bool = commands.Param(name="скрыть плейлист",
    ...
    async def volume(
            self,
            inter: disnake.AppCmdInter, *,
            value: int = commands.Param(name="громкость", description="громкость от 5 до 150", min_value=5.0, max_value=150.0)
    ):
    ...
    async def seek(
            self,
            inter: disnake.AppCmdInter,
            position: str = commands.Param(name="время", description="Время перехода вперед/назад (например: 1:45 / 40 / 0:30)",
